...
- Compared kernel structures and regularization approaches  
- Integrated Bayesian Optimization for hyperparameter tuning and exploration strategies  
- Evaluated robustness under noisy, non-stationary market conditions  
...
#### Machine Learning / Quant Libraries  
`NumPy` • `Pandas` • `SciPy` • `Scikit-Learn`  
`GPy` • `GPflow` • `PyTorch` • `Statsmodels` • `TA-Lib`

#### Tools & Environments  
`Git` • `GitHub` • `Jupyter` • `PyCharm` • `LaTeX` • Bloomberg Terminal *(exposure)*  
